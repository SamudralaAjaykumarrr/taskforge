...
| **Retry recovery** | Fraction of jobs that eventually reach `SUCCEEDED` after at least one `RETRY_WAIT` cycle, vs. those that exhaust to `DEAD_LETTERED`. | **Cannot be computed by joining `taskforge_retry_count` (a histogram) with `taskforge_jobs_completed_total{outcome=...}` (a counter)** — neither aggregate carries per-job identity, so there is no key to join on; that pairing looks plausible but does not actually answer the question. Compute this instead from authoritative durable state — a query over `jobs`/`job_attempts` (e.g., `COUNT(*) WHERE state='SUCCEEDED' AND attempt_count > 1` vs. `COUNT(*) WHERE state='DEAD_LETTERED'`) — or, if a live SLI is wanted, add a dedicated bounded-cardinality counter (e.g., `taskforge_jobs_recovered_after_retry_total`, incremented once per job at its terminal transition) rather than attempting to derive the answer from existing per-attempt aggregates. | No target proposed — this is workload-dependent (a job type with a high permanent-failure rate is a data problem, not an availability problem) |
...
| **PostgreSQL CPU / activity** | **Core `pg_stat_*` views (`pg_stat_activity`, `pg_stat_statements`) do not directly report host CPU consumption** — they report query/session/statement-level activity, not CPU time. Host CPU must come from OS/container/cgroup monitoring (e.g., `/proc/stat`, cgroup CPU accounting, or the cloud provider's host-level metric) run alongside a `cmd/chaos` run. `pg_stat_activity`/`pg_locks` remain the right source for database-side contention (active queries, wait events, lock waits), and `pg_stat_statements` (if enabled) for per-query time/call-count aggregates — but none of the three substitutes for host CPU measurement. | No integration with any Postgres-side or host-level monitoring exists or is proposed as TaskForge's own responsibility (correctly — this is infrastructure monitoring, not application instrumentation) but a capacity test plan must explicitly include both host-CPU monitoring and `pg_stat_activity`/`pg_locks` as external measurements, not treat one as a substitute for the other. |
...
5. A deliberate overload run (submission rate held above sustainable claim
   ...
   than assumed.
6. External monitoring run alongside every test above: OS/container/cgroup-
   level CPU measurement (not available from `pg_stat_*`) plus
   PostgreSQL-side `pg_stat_activity`/`pg_locks`/index-bloat monitoring.
