...
func TestChaos_ObservabilityDuringFailure_MetricsReflectEveryFaultClass(t *testing.T) {
	...
	require.NoError(t, err)
	claimed, ok, err := claimUntilResolved(t, ctx, s, "obs-w1")
	require.NoError(t, err)
	require.True(t, ok)
	require.NoError(t, chaos.ForceExpireLease(ctx, db, claimed.ID))
	reclaimed, ok, err := claimUntilResolved(t, ctx, s, "obs-w2")
	require.NoError(t, err)
	require.True(t, ok)
	...
	require.NoError(t, err)
	c1, ok, err := claimUntilResolved(t, ctx, s, "obs-dlq-w1")
	require.NoError(t, err)
	require.True(t, ok)
	...
	c2, ok, err := claimUntilResolved(t, ctx, s, "obs-dlq-w2")
	require.NoError(t, err)
	require.True(t, ok)
	...
	require.NoError(t, err)
	tc, ok, err := claimUntilResolved(t, ctx, s, "obs-timeout-w")
	require.NoError(t, err)
	require.True(t, ok)
...
func TestChaos_ObservabilityDuringFailure_CardinalityBoundedUnderManyJobs(t *testing.T) {
	...
	for i := 0; i < numJobs; i++ {
		...
		require.NoError(t, err)
		claimed, ok, err := claimUntilResolved(t, ctx, s, fmt.Sprintf("obs-card-worker-%d", i))
		require.NoError(t, err)
		require.True(t, ok)
